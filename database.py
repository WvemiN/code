...
def list_all_cards():
    ...
    cur = conn.cursor()
    try:
        sql = """SELECT * FROM opaltravel.opalcards;"""
...
def add_new_card(cardtypeid, userid, expiry, balance):
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        ...
        print("Unexpected error adding a card:", e)
    finally:
        cur.close()
        conn.close()

def update_card(cardid, userid, cardtypeid):
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        sql = """
        UPDATE opaltravel.opalcards
        SET userid = %s, cardtypeid = %s
        WHERE cardid = %s;
        """
        cur.execute(sql, (userid, cardtypeid, cardid))
        conn.commit()
    except Exception as e:
...
def delete_card(cardid):
    ...
    try:
        sql = """
        DELETE FROM opaltravel.opalcards
        WHERE cardid = %s;
        """
        cur.execute(sql, (cardid,))
        conn.commit()
    except Exception as e:
        print("Unexpected error deleting card with id ", cardid, e)
    finally:
        cur.close()
        conn.close()

def cards_expiry_report():
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
